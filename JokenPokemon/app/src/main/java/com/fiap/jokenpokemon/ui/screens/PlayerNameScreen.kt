...
fun PlayerNameScreen(
    onStartGameNavigateTo: (String) -> Unit
) {
    ...
    Column(
        ...
        PokeLogo(R.drawable.logo_pokemon, "Pokemon", 120.dp)
        ...
        Button(
            onClick = {
                val name = playerNameInput
                Log.d("DEBUG Player Name", name)
                onStartGameNavigateTo(name)
            },
            shape = CircleShape,
            enabled = playerNameInput.isNotEmpty(),
            modifier = Modifier.fillMaxWidth(),
...
@Composable
fun PlayerNameScreenPreview() {
    JoKenPokemonTheme {
        PlayerNameScreen { "Ash" }
    }
}